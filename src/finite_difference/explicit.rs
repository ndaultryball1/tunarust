...
fn explicit_fwd<T: Vanilla + Discretisable>(
    ...
    let dt = to_price.dimless_time(&underlying, time_remaining) / M as f64;
    let alpha = dt / (DX * DX);
    ...
    let mut newu: Vec<f64> = (0..NUMX).map(|x| x as f64).collect();

    // Set temporal boundary conditions
    let mut oldu = (0..NUMX)
        .map(|x| {
            let price = (x + MINUS) as f64 * DX;
    ...
    for j in 1..M {
        ...
        *newu.first_mut().unwrap() =
            to_price.boundary_spatial_m(&underlying, MINUS as f64 * DX, tau);
